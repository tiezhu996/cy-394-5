...
func New(db *gorm.DB, logger *zap.Logger, cfg config.Config) *gin.Engine {
	...
	userSvc := service.NewUserService(userRepo, friendshipRepo)
	records := handler.NewRecordHandler(recordSvc)
	stats := handler.NewStatsHandler(recordSvc)
	rank := handler.NewRankHandler(recordSvc, userSvc)
	...
	api.GET("/user/profile", users.Profile)
	api.POST("/friends/add", users.AddFriend)
	api.GET("/friends", users.ListFriends)
	return app
}
